...
function verificarSumaPreguntas() {
  ...
  console.log('Preguntas:', ch2.preg_1, ch2.preg_2, ch2.preg_3, ch2.preg_4, 'Suma:', suma, 'Intento:', gameAttempts + 1);
  ...
  if (suma === 4) {
    $('#slide_ok_1').show();
    console.log('[Game] Todas las respuestas correctas, mostrando modal de aprobación');
  } else {
    gameAttempts++; // Incrementar el contador de intentos
    if (gameAttempts < maxGameAttempts) {
      $('#slide_error_1').show();
      console.log('[Game] Respuestas incorrectas, mostrando modal de error, Intento:', gameAttempts);
    } else {
      // Después de 3 intentos, avanzar a la diapositiva 13 sin mostrar modal
      console.log('[Game] Límite de intentos alcanzado, avanzando a diapositiva 13');
      advanceToSlide13();
    }
  }
}
...
function advanceToSlide13() {
  $('#slide_error_1').hide(); // Asegurar que el modal incorrecto esté oculto
  $('#juego2').hide().empty(); // Ocultar y limpiar el contenedor del juego (si aplica)
  nSlides.numSlides_2 = 13; // Avanzar a la diapositiva 13
  if (myAvance.ch2.logro_zapatos === 0) {
    myAvance.ch2.logro_zapatos = 1;
    if (myAvance.ch2.progress < 4) {
      myAvance.ch2.progress = 4; // Unlock Cierre
...
$('#btn_cls_ok_modal').click(function () {
  $('#slide_ok_1').hide();
  console.log('[Game] Aprobado, avanzando a diapositiva 13');
  advanceToSlide13();
});
...
$('#btn_cls_error_modal').click(function () {
  ...
  if (gameAttempts < maxGameAttempts) {
    ...
    console.log('[Game] Reintentando juego, intento:', gameAttempts + 1);
    ctrl_slidesMod2();
  } else {
